...
int utils::GetRandom(int begin, int end)
{
	return rand() % (end - begin + 1) + begin;
}
Color4f utils::GetRandomColor()
{
	return Color4f{ rand() % 256 / 255.0f, rand() % 256 / 255.0f, rand() % 256 / 255.0f, 1.0f };
}
int utils::GetIndex(int rowIdx, int colIdx, int nrCols)
{
	return rowIdx * nrCols + colIdx;
}
int utils::GetColumn(int arrayIdx, int nrCols)
{
	return arrayIdx % nrCols;
}
int utils::GetRow(int arrayIdx, int nrCols)
{
	return arrayIdx / nrCols;
}